...
} from "./moonshine-constants";
import { supportsWebGPU } from "./moonshine-utils";

// const device = (await supportsWebGPU()) ? "webgpu" : "wasm";
const device = "wasm";
self.postMessage({ type: "progress", file: "device-selection", progress: 10, total: 100, name: "Selecting device" });
console.warn('[MoonshineWorker] Device selected:', device);
self.postMessage({ type: "log", message: `Device selected: ${device}` });
self.postMessage({ type: "info", message: `Using device: "${device}"` });
// Send loading status for UI loader
...
console.warn('[MoonshineWorker] Loading models...');
self.postMessage({ type: "log", message: "Loading models..." });
self.postMessage({ type: "progress", file: "silero-vad", progress: 20, total: 100, name: "Loading Silero VAD" });

// Progress callback for accurate, real-time progress reporting
function progress_callback(progress) {
  // Attach a file identifier if not present
  if (!progress.file) progress.file = progress.name || "unknown";
...
try {
  ...
  console.warn('[MoonshineWorker] Silero VAD model loaded');
  self.postMessage({ type: "log", message: "Silero VAD model loaded" });
  self.postMessage({ type: "progress", file: "silero-vad", progress: 40, total: 100, name: "Silero VAD loaded" });
} catch (error) {
  console.error('[MoonshineWorker] Error loading Silero VAD:', error);
  self.postMessage({ type: "log", message: `Error loading Silero VAD: ${error}` });
...
self.postMessage({ type: "progress", file: "transcriber", progress: 50, total: 100, name: "Loading Transcriber" });
let transcriber;
try {
  ...
  console.warn('[MoonshineWorker] Transcriber model loaded');
  self.postMessage({ type: "log", message: "Transcriber model loaded" });
  self.postMessage({ type: "progress", file: "transcriber", progress: 70, total: 100, name: "Transcriber loaded" });
} catch (error) {
  console.error('[MoonshineWorker] Error loading transcriber:', error);
  self.postMessage({ type: "log", message: `Error loading transcriber: ${error}` });
  self.postMessage({ error });
  throw error;
}

self.postMessage({ type: "progress", file: "pipeline-compile", progress: 80, total: 100, name: "Compiling pipeline" });
try {
  await transcriber(new Float32Array(SAMPLE_RATE)); // Compile shaders
  console.warn('[MoonshineWorker] Transcriber model compiled and ready');
  self.postMessage({ type: "log", message: "Transcriber model compiled and ready" });
  self.postMessage({ type: "progress", file: "pipeline-compile", progress: 100, total: 100, name: "Pipeline ready" });
  self.postMessage({ type: "status", status: "ready", message: "Ready!" });
} catch (error) {
  console.error('[MoonshineWorker] Error compiling transcriber:', error);
  self.postMessage({ type: "log", message: `Error compiling transcriber: ${error}` });
  self.postMessage({ error });
  throw error;
}
...
self.onmessage = async (event) => {
  // console.log('[MoonshineWorker] Received event:', event);
  self.postMessage({ type: "log", message: "Received event in worker" });
